...
class Application {
	...
	public function __construct($config = array()) {
		...
		require_once('Moongrace' . DIRECTORY_SEPARATOR . 'Lib' . DIRECTORY_SEPARATOR . $key . EXT);
	}
	...
	public function loadView($name = '', $param = array()) {
		$file = APP . DIRECTORY_SEPARATOR . 'Views' . DIRECTORY_SEPARATOR . $name . EXT;
		if(file_exists($file)) {
			extract($param);
			require_once ($file);
		} else
	...
	public function loadLib($name = '', $param = array()) {
		$file = 'Moongrace' . DIRECTORY_SEPARATOR . 'Lib' . DIRECTORY_SEPARATOR . $name . EXT;
		if(file_exists($file)) {
			require_once ($file);
			if(empty($param)) {
